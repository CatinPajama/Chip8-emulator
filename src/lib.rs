...
pub mod emulator {
    ...
    fn nibbles(u : u16) -> (u16, u16, u16, u16) {
        ((u & 0xF000) >> 12, (u & 0x0F00) >> 8, (u & 0x00F0) >> 4, u & 0x000F)
    }
            ...
            for row in 0..self.rows {
                for col in 0..self.cols {
                    if self.pixels[row * self.cols + col] {
                        draw_rectangle((col * self.pixel_size) as f32, (row * self.pixel_size) as f32, (self.pixel_size) as f32, self.pixel_size as f32, WHITE)
                    }
                }
            }
        }
...
//                self.memory[addr] = program[addr - 0x200];
            ...
            let v = [0xF0, 0x90, 0x90, 0x90, 0xF0,
        ...
        pub fn run(&mut self) {
            ...
            self.screen.draw();
            let ins = ((self.memory[self.pc as usize] as u16) << 8) | (self.memory[self.pc as usize + 1 ]) as u16;
        ...
        fn opDxyn(&mut self, x : usize, y : usize, n : u8) {
            ...
            for byte in 0..n {
                for bit in 0..8 {
                    let pixel = (self.memory[self.registers.i as usize + byte as usize] >> (7 - bit)) & 1;
                    self.registers.v[0xf] |= self.screen.set(((self.registers.v[y] as u16) + byte as u16) as usize, ((self.registers.v[x] as u16) + (bit as u16)) as usize, pixel == 1);
                }
            }
